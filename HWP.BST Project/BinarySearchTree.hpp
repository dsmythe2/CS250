    ...
    void Delete( const TK& key )
    {
		...
		if (thisNode != nullptr)
		{
			...
			if (thisNode->ptrLeft != nullptr && thisNode->ptrRight != nullptr)
			{
				...
				Node<TK, TD>* nextNode = thisNode;
				Node<TK, TD>* nextNodeParent = thisNode;
				...
				if (nextNode != thisNode->ptrRight)
				{
					nextNodeParent->ptrLeft = nextNode->ptrRight;
					nextNode->ptrRight = thisNode->ptrRight;
				}
				...
				else if (toLeft)
				{
					...
					parentNode->ptrLeft = nextNode;
				}
				else
				{
					thisNode = parentNode->ptrRight;
					parentNode->ptrRight = nextNode;
				}
				nextNode->ptrLeft = thisNode->ptrLeft;
			}
...
private:
    ...
    */
    int GetHeight( Node<TK, TD>* ptrCurrent )
    {
		...
		{
			int left, right = 0;
			left = GetHeight(ptrCurrent->ptrLeft);
			right = GetHeight(ptrCurrent->ptrRight);
